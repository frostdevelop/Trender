...
function looprender() {
	...
	if(gravity){
		y += fallCurve;
		fallCurve += 2;
		y>=0 && (fallCurve=-(fallCurve/2));
		Math.abs(fallCurve)<=10 && Math.abs(y)<=100 && (gravity=false,y=0);
	}
	renderscene(cubes)
	animate(cubes)
...
function animate(scene){
	//scene[1][1][1] <= -2000 && (animateVar = 10);
	scene[1][1][1] >= -700 && (animateVar = -100);
	animateVar += 2;
	scene[1][1][1] += animateVar;
}
...
fovElm.addEventListener('input',()=>screendist=fovElm.value);
